...
public class NormalizedOpenIdAttributesBuilder {
	...
	public NormalizedOpenIdAttributes build(
		...
		return new NormalizedOpenIdAttributes(userLocalIdentifier,
				emailAddress, fullName, loginReplacement);
	}

	private String setUpLoginReplacement(
			OpenIDAuthenticationToken openIdAuthenticationToken) {
		String separator = ".";
		return getNameFromFirstAndLast(openIdAuthenticationToken, separator)
				.toLowerCase();
	}
	...
	private String setUpFullName(
			OpenIDAuthenticationToken openIdAuthenticationToken) {
		String fullName = getAttributeValue(openIdAuthenticationToken,
				fullNameAttributeNames);
		if (fullName == null) {
			String separator = " ";
			fullName = getNameFromFirstAndLast(openIdAuthenticationToken,
					separator);
		}
	...
	private String getNameFromFirstAndLast(
			OpenIDAuthenticationToken openIdAuthenticationToken,
			String separator) {
		String firstName = getAttributeValue(openIdAuthenticationToken,
				firstNameAttributeNames);
		String lastName = getAttributeValue(openIdAuthenticationToken,
				lastNameAttributeNames);
		return StringUtils
				.join(new String[] { firstName, lastName }, separator);
	}
