...
public class CategoryEntityView extends CustomComponent {
	...
	private final Table expertTable = setupNewTable(ThemeConstants.TAB_CAPTION_EXPERTS, ThemeConstants.TAB_ICON_EXPERTS);	
	...
	public CategoryEntityView(Category c, TabNavigator navigator){
		...
		expertContainer.addContainerFilter(new Equal("category", sourceCategory));
		...
		propositionContainer.addContainerFilter(new Equal("categories", sourceCategory));
	...
	private VerticalLayout buildScrollContent() {
		...
		if(expertContainer != null){
			...
			expertTableGeneratedColumns.put("Username", new ExpertColumnGenerator("username"));
			expertTableGeneratedColumns.put("Average Trust", new ExpertColumnGenerator("representation"));
			expertTableGeneratedColumns.put("Supporters", new ExpertColumnGenerator("supporters"));
			expertTableGeneratedColumns.put("Average Distrust", new ExpertColumnGenerator("distrust"));
			expertTableGeneratedColumns.put("Opposers", new ExpertColumnGenerator("opposers"));
			
		    Iterator<Map.Entry<String, ExpertColumnGenerator>> it = expertTableGeneratedColumns.entrySet().iterator();
		    while (it.hasNext()) {
    ...
    private class ExpertColumnGenerator implements Table.ColumnGenerator {
        ...
        public Component generateCell(final Table source, final Object itemId, Object columnId) {
        	...
        	if(column == "representation"){
	        	...
	        	generatedCell = new Label((rep == null) ? "0" : Integer.toString(Math.round(rep.floatValue())));
    ...
    private class PropositionColumnGenerator implements Table.ColumnGenerator {
        ...
        public Component generateCell(Table source, Object itemId, Object columnId) {
        	final Proposition proposition = propositionContainer.getItem(itemId).getEntity();
        	if (column == "username"){
