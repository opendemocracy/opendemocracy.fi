...
public class ODUserView extends
	...
	protected CustomComponent createView(String uri) {
		...
		if (uri != null) {
			...
			try {
				...
				entity = getEntityForItem(getTable().getItem(id));
			} catch (NumberFormatException e) {

			}
		}
		if (entity == null) {
			entity = getEntityForItem(getTable().getItem(getTable().getValue()));
		}
		Object user = getApplication().getUser();
		if (user != null && user instanceof ODUser
				&& ((ODUser) user).getId().equals(entity.getId())) {
	...
	protected void configureTable(Table table) {
		...
		table.setVisibleColumns(new Object[] { "id", "username", "firstName",
				"lastName" });
