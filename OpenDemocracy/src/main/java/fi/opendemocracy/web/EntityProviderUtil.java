...
/**
 * Utility class for constructing/obtaining a shared entity provider instance
 * for an entity class.
 */
public class EntityProviderUtil {
	...
	private <T> MutableLocalEntityProvider<T> buildEntityProvider(Class<T> cls) {
		...
		if (null != entityManager) {
			MutableLocalEntityProvider<T> provider = new MutableLocalEntityProvider<T>(
					cls, entityManager);
	...
	private EntityManager getEntityManager(Class<?> cls) {
		try {
			...
			throw new IllegalArgumentException(
					"Could not obtain entity manager for the entity class "
							+ cls.getCanonicalName(), e);
		}
		throw new IllegalArgumentException(
				"Could not obtain entity manager for the entity class "
						+ cls.getCanonicalName()
						+ ": missing static method entityManager()");
	}
}
